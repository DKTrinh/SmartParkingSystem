...
const IOTManagement = () => {
    ...
    const fetchDevices = async (keyword = "") => {
        try {
            const url = keyword 
                ? `http://localhost:5000/api/devices?keyword=${encodeURIComponent(keyword)}` 
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const url = isEditing 
            ? `http://localhost:5000/api/devices/${formData.id}` 
            : "http://localhost:5000/api/devices";
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa thiết bị này?")) {
            try {
                const response = await fetch(`http://localhost:5000/api/devices/${id}`, { method: "DELETE" });
                if (response.ok) {
                    fetchDevices();
                } else {
