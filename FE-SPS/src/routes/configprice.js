...
const ConfigPrice = () => {
    ...
    const loadConfigPrices = async () => {
        ...
        try {
            const res  = await axios.get(`${apiUrl}/api/pricing`);
    ...
    const handleDeleteConfig = async (id) => {
        ...
        try {
            await axios.delete(`${apiUrl}/api/pricing/${id}`);
        ...
        try {
            await Promise.all(changed.map(item =>
                (item.isNew || `${item.id}`.startsWith('new-'))
                    ? axios.post(`${apiUrl}/api/pricing`, buildPayload(item))
                    : axios.put(`${apiUrl}/api/pricing/${item.id}`, buildPayload(item))
            ));
